...
import { day, time } from '@/utils/datetime';
...
export default function TableRow({ item }) {
  ...
  return (
    <Table.Tr key={item.id}>
      ...
      <Table.Td>
        <Anchor onClick={() => redirectTo('clients.companies.edit', item.client_company_user.id)}>
          <Text
            fz='md'
            fw={600}
          >
            {item.client_company_user.name}
          </Text>
        </Anchor>
        <Text
          fz='xs'
      ...
      <Table.Td>
        <Text fz='sm'>{money(item.amount, item.client_company_user.currency.code)}</Text>
      </Table.Td>
      <Table.Td>
        <Text fz='sm'>{money(item.amount_with_tax, item.client_company_user.currency.code)}</Text>
      </Table.Td>
      <Table.Td>
        <Text
          fz='sm'
