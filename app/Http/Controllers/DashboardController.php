        ...
        return Inertia::render('Dashboard/Index', [
            'projects' => Project::whereIn('id', $projectIds)
                ...
                ->withCount([
                    ...
                    'tasks AS overdue_tasks_count' => fn ($query) => $query->whereNull('completed_at')->whereDate('end_date', '<', now()), // Ganti 'due_on' dengan 'end_date'
                ])
            ...
            'overdueTasks' => Task::whereIn('project_id', $projectIds)
                ...
                ->whereDate('start_date', '<', now(), 'or')
                ->whereDate('end_date', '<', now())
                ->where('assigned_to_user_id', auth()->id())
                ->with('project:id,name')
                ->with('taskGroup:id,name')
                ->orderBy('start_date')
                ->get(['id', 'name', 'start_date', 'end_date', 'group_id', 'project_id']),
